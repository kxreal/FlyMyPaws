...
const Login = () => {
  ...
  // Check if admin has disabled email registration
  useEffect(() => {
    const fetchSetting = async () => {
      const { data } = await supabase
        .from('app_settings')
        .select('value')
        .eq('key', 'email_registration_enabled')
        .single();
      ...
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
          ...
          <h1 style={{ fontSize: '1.5rem', marginBottom: '0.3rem' }}>
            {tab === 'login' ? 'Welcome back' : 'Join FlightFurward'}
          </h1>
            ...
            <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '0.8rem', color: 'var(--color-primary)', cursor: 'pointer' }}>Forgot password?</span>
                </div>
